...
| `portal/factura` + `tasks/factura` | MIPYME facturación gratuita — BORRADORES only, never emission (ADR-023). Empresa-keyed (`mipeSelEmpresa.cgi` list). JS-populated form driven via `goto`/`evaluate`; borrador CRUD via `requestForm`; listing via `requestJson`; preview PDF via `requestBinary` + `FileSink` | Done |

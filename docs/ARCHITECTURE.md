...
- `SecretStore` — read the Clave from the OS keyring (default:
  `KeyringSecretStore` over `@napi-rs/keyring`; ADR-025). Optional on `Runtime`.
...
| `tasks/*` | High-level operations the surfaces call. Document-producing tasks (`f29Pdf`) write via `FileSink` and return a descriptor, never bytes (ADR-022). Domain reads wrap a facade in `withSession` + one audit receipt (auth, operate, rcv, f22, f29, bte done; profile, iva, renta planned) — except `dteAuthorized` (public): NO `withSession`, still audited (ADR-014) | auth/operate/rcv/f22/f29/dte/bte done; rest planned |
| `portal/dte-mipyme` (+ `tasks/dte`, the MIPYME half of the `dte` artifact — ADR-024) | MIPYME facturación gratuita — BORRADORES only, never emission (ADR-023). Empresa-keyed (`mipeSelEmpresa.cgi` list). JS-populated form driven via `goto`/`evaluate`; borrador CRUD via `requestForm`; listing via `requestJson`; preview PDF via `requestBinary` + `FileSink`. Also READ-ONLY `emitidos` (documentos emitidos listing, malformed-table parser) + `pdf` (the emitted document, plain GET) | Done |
